// Code from Plugin Browser Plugin or nexpid GitHub 

import { findByName, findByProps } from "@vendetta/metro";
import { i18n, lodash, React, ReactNative as RN, stylesheet, NavigationNative} from "@vendetta/metro/common";
...
export function patchSettingsPin(
	...
	const unpatch = after("default", settingsModule, (_, ret) => {
		...
		patches.push(
			after("render", Overview.type.prototype, (_, { props: { children } }) => {
				const titles = [
					i18n.Messages["BILLING_SETTINGS"],
					i18n.Messages["PREMIUM_SETTINGS"],
				];
				
				children = findInReactTree(children, (t) => t.children[1].type === FormSection).children;

				const index = children.findIndex((c: any) => titles.includes(c?.props.label));

				if (shouldAppear()) children.splice(index === -1 ? 4 : index, 0, render({}));
			}),
		);
	}, true);
	...
	if (getScreens && you) {
		...
		const component = React.memo(({ navigation }: any) => {
			managePage(without(you.page, "noErrorBoundary", "render") as any, navigation );
			return (
		...
		const manipulateSections = (ret: any, nw?: boolean) => {
			...
			const title = "Bunny";
			const section = sections.find((x) => x?.title === title || x?.label === title);

			if (section && !section?.settings?.includes(screenKey)) section.settings.push(screenKey);
		...
		const oldYouPatch = () => {
			...
			patches.push( 
				after("useOverviewSettings", layout, (_, ret) => manipulateSections(ret))
			);
			
			patches.push(
				after("getSettingTitleConfig", titleConfig, (_, ret) => ({...ret, [screenKey]: you.title })),
			);
			...
			stuff.SETTING_RELATIONSHIPS = { ...oldRelationships, [screenKey]: null};
			stuff.SETTING_RENDERER_CONFIGS = { ...oldRendererConfigs, ...rendererConfig };
		...
		const newYouPatch = () => {
			...
			patches.push(
				before("type", settingsListComponents.SearchableSettingsList, (ret) => manipulateSections(ret, true)),
			);

			patches.push(
				after("getSettingListSearchResultItems", gettersModule, (_, ret) => {
					for (const s of ret) if (s.setting === screenKey) s.breadcrumbs = ["Bunny", "Nexpid"];
				}),
			);
